...
const CreateWordModal = ({ jwtToken, onClose, onSave }: CreateWordModalProps) => {
  const [languageLevels, setLanguageLevels] = useState<LanguageLevel[]>([]);
  const [topics, setTopics] = useState<Topic[]>([]);
  const [tags, setTags] = useState<UserTag[]>([]);
  const [wordDto, setWordDto] = useState<WordDto>({ 
    ...
    Tags: [''],
    WordForms: [],
    WordExamples: [{ Guid: "00000000-0000-0000-0000-000000000000", OriginalExample: '', TranslatedExample: '' }],
  });
  ...
  const handleAddWord = async () => {
    try {
      ...
      console.error(err);
      alert('Ошибка при создании слова');
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-md space-y-4">
        <h2 className="text-2xl font-semibold">Создать новое слово</h2>
